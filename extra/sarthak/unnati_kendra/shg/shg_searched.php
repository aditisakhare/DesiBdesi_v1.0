...
<form method="GET" action="shg_searched.php">
    ...
    <div class="col-md-3"><input type="submit" value="Search" class="btn btn-default"></div>
...
<?php
if (empty($_GET)) {
    echo "<div class='container'>No Match Found</div>";
}
else
{
...
$sql_buyer_adv_post = "SELECT * FROM buyer_adv_post where item_id = $item_id ORDER BY item_id <> $shg_item_id, quantity DESC ";
$res_buyer_adv_post = mysqli_query($connect, $sql_buyer_adv_post);
if(mysqli_num_rows($res_buyer_adv_post)==0)
{
    echo "<div class='container'>No Match Found</div>";
}
else {
